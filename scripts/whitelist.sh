...
GITHUB_WHITELIST_LOCATION="$(pwd | sed s/scripts/domains/g)"
...
curl -sS https://raw.githubusercontent.com/anudeepND/whitelist/master/domains/whitelist.txt | sudo tee -a "${GITHUB_WHITELIST_LOCATION}"/whitelist.txt >/dev/null
cat "${GITHUB_WHITELIST_LOCATION}"/whitelist.txt | sudo tee -a "${PIHOLE_LOCATION}"/whitelist.txt >/dev/null

...
then
	...
	mv "${PIHOLE_LOCATION}"/whitelist.txt /etc/pihole/whitelist.txt.old && sort -u "${PIHOLE_LOCATION}"/whitelist.txt.old >> "${PIHOLE_LOCATION}"/whitelist.txt

	wait
	echo -e " [...] \e[32m Pi-hole gravity rebuilding lists. This may take a while \e[0m"
	${GRAVITY_UPDATE_COMMAND} $(xargs < /etc/pihole/whitelist.txt) > /dev/null
	wait

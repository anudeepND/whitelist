    ...
    try: # Try to create a DB connection
        ...
        for userAddINgravity in userAddTUP: # for every whitelisted domain we found in the database do:
           if userAddINgravity[2] in newWhiteList: # if the domain we found added by user IS IN our new list count it
               ...
               uagl = True
               uA = uA + 1 # bump count if domain added (starts @ 0)
        #
        uA = uA -1 # subtract 1 so the count doesn't put us out of range
        ...
        if uagl == True: # if we found user added domains from our list in gravity do:
            ...
            while uA >= 0: # remember that counter now we make it go backwards to 0
                a = a + 1 # counter for number output to screen
                print ('    {}. {}' .format(a, userAddList[uA])) # Show us what we found
                uA = uA - 1 # go backwards
        ...
        print('[i] Checking Gravity database for domains previously added by script that are NOT in new script')
        ignl = False
        a = 0
        for INgravityNOTnew in gravScriptBeforeTUP: # for every domain previously added by script 
            ...
            a = a + 1
            if not INgravityNOTnew[2] in newWhiteList: # make sure it is not in new script or added by user
               INgravityNOTnewList[z] = INgravityNOTnew[2] # 
               ignl = True
               z = z + 1
        #
        z = z -1
        ...
        if ignl == True:
            ...
            while z >= 0:
                a = a + 1
                print ('    {}. {}' .format(a, INgravityNOTnewList[z]))
                #  ability to remove old
                # uncomment the next 2 lines to delete domains not in script anymore
        ...
        if ignl == True:
            print('\n')
        print('[i] Checking script for domains not in database')
        ilng = False
        ...
        if ilng == True: # Add domains that are missing from new script and not user additions
            print ('[i] {} domain(s) NOT in Gravity that are in new script.' .format(INnewNOTgravityListCount+1))
            a = 0
            while w >= 0:
                a = a + 1
                for addNewWhiteDomain in newWhiteList:
                    if addNewWhiteDomain in INnewNOTgravityList:
                        print ('    - Adding {} to database' .format(addNewWhiteDomain))
                        sql_index = newWhiteList.index(addNewWhiteDomain)
                        # ability to add new
                        # uncomment the next 2 line to add domains not in gravity yet
            ...
            while ASG >= 0:
                if INnewNOTgravityList[ASG] in gravScriptAfterList:
                    print('    - Found  {} in database' .format(INnewNOTgravityList[ASG]))
                    gsa = True
                ASG = ASG - 1

            if gsa == True:
                # Do Nothing and exit. All domains are accounted for.
                print ("\n[i] All Missing Domains have been Added!")
                print("[i] All {} domains to be added by script have been discovered in database" .format(newWhiteListlen))

            else:
                print ("\n[i] {} New Domain(s) not added to database" .format(INnewNOTgravityListCount+1))
